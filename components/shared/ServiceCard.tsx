...
export function ServiceCard({ slug, title, excerpt, icon, className }: ServiceCardProps) {
  return (
    ...
    >
      {icon && (
        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-securenext-accent/20 text-securenext-accent group-hover:bg-securenext-accent group-hover:text-white transition-all duration-300">
          {icon}
        </div>
      )}
